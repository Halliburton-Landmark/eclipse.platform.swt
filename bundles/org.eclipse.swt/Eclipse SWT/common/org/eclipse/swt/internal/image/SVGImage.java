...
public class SVGImage extends BufferedImage {
    ...
    private static final Map<Integer, String> scaleQuality = new HashMap<>();
    static {
        String css = "svg {" +
                     "shape-rendering: %s;" +
                     "text-rendering:  %s;" +
    ...
    public static SVGImage fromSvg(URL resource, int size) throws IOException {
        InputStream rs = null;
        try {
            rs = resource.openStream();
            SVGDocument svg = factory.createSVGDocument(resource.toString(), rs);
            return fromSvgDocument(resource, svg, size);
        } finally {
            if (rs != null) {
                try {
                    rs.close();
    ...
    public static SVGImage fromSvgDocument(URL resource, SVGDocument doc, int size) {
        return new SVGImage(resource, doc, size, size, SCALE_DEFAULT);
    }
